...
# 5. Save logic at the end of `sendMessage` - only ONE replace by targeting the end of sendMessage!
# Let's find a safe hook. 
# In `sendMessage`, there are multiple `_isGenerating.value = false` due to try/catch blocks.
# I'll just replace `_isGenerating.value = false` with a call to `syncMemory()`, and add `syncMemory()` method.
...
"""
content = content.replace("fun clearMessages() {", sync_memory + "\n    fun clearMessages() {")
content = content.replace("_isGenerating.value = false", "syncMemory()")
