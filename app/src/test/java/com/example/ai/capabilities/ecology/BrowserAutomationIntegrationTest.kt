...
class BrowserAutomationIntegrationTest {
    ...
    @Test
    fun testBrowserAutomationRouting() = runBlocking {
        // 1. Initialize Kernel and Mock Dependencies
        val repo = MockJobStateRepository()
        val kernel = CognitiveKernelImpl(repo, "job-456", CognitiveState.QUEUED)
        ...
        // 3. Define the Task
        // We simulate a browser automation task. On the daemon side it just runs the command.
        val commandToRun = "echo 'Browser Automation Evidence'"
        val task = AutonomousWorkerTask(
            taskId = "task-browser-01",
            goal = "Verify Browser Automation Task Routing",
            role = WorkerRole.TERMINAL,
            parameters = mapOf(
                "command" to commandToRun,
                "requiresBrowserAutomation" to "true"
            ),
            context = ""
        )
        ...
        assertTrue("Output should contain the evidence string", result.output.contains("Browser Automation Evidence"))
        
        assertEquals(CognitiveState.COMPLETED, kernel.currentState)
        assertEquals(CognitiveState.COMPLETED.name, repo.getJobState("job-456"))
